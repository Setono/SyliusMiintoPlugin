...
final class ShippingMethodMappingType extends AbstractResourceType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
            ->add('providerId', TextType::class, [
                'label' => 'setono_sylius_miinto.form.shipping_method_mapping.provider_id',
                'disabled' => true,
            ])
            ->add('shop', TextType::class, [
                'label' => 'setono_sylius_miinto.form.shipping_method_mapping.shop',
                'disabled' => true,
            ])
        ;
        ...
        $builder->addEventListener(FormEvents::POST_SET_DATA, static function (FormEvent $event) {
            ...
            if (null !== $channel) {
                ...
                $form->add('shippingMethod', ShippingMethodChoiceType::class, [
                    'label' => 'setono_sylius_miinto.form.shipping_method_mapping.shipping_method',
                    'placeholder' => 'setono_sylius_miinto.form.shipping_method_mapping.choose_shipping_method',
                    'channel' => $channel,
                ]);
            }
        });
    ...
    {
        return 'setono_sylius_miinto_shipping_method_mapping';
    }
}
